    ...
    private val seedData = listOf(
        SeedCategory(
            ...
            questions = listOf(
                ...
                SeedQuestion(
                    "RAG 与微调的区别？如何选择？",
                    "RAG 不修改模型权重，通过实时注入检索到的外部知识更新信息，适合知识频繁变化、需要可溯源（引用来源）的场景。\n\n微调把知识或风格写入权重，适合固定领域、指令遵循或风格定型，但知识更新需重新训练且无法溯源。\n\n实践中常组合使用：用 RAG 解决知识来源，再用微调对齐输出风格。",
                    2,
                ),
                SeedQuestion(
                    "什么是重排序（Rerank）？在 RAG 中的作用？",
                    "重排序是对初步检索出的 Top-K 候选做更精细的相关度打分并重新排序，通常用交叉编码器（cross-encoder）逐对计算查询-文档相关性，比 embedding 双塔检索更准确。\n\n重排序后的 Top-N 再送入 LLM，能显著提升生成质量。",
                    2,
                ),
                SeedQuestion(
                    "什么是查询改写（Query Rewriting）？为什么需要？",
                    "用户原始查询常口语化、指代不清或多轮依赖。查询改写由 LLM 将原始查询改写为更利于检索的表述：\n\n1) 补全指代；\n2) 提取关键词；\n3) 分解子问题；\n4) 生成假设性答案（HyDE）。\n\n从而提升召回质量，是 RAG 的常见优化手段。",
                    2,
                ),
                SeedQuestion(
                    "RAG 常用的评估指标有哪些？",
        ...
        SeedCategory(
            ...
            questions = listOf(
                SeedQuestion(
                    "什么是 AI Agent？核心组成部分？",
                    "AI Agent 是基于 LLM 的自主智能体，能感知环境、基于目标推理决策并执行行动。\n\n核心组成：\n\n1) LLM（大脑）；\n2) 工具（函数调用/API，能力）；\n3) 规划（拆解任务）；\n4) 记忆（短期上下文 + 长期存储）；\n5) 执行与反馈循环。",
                    1,
                ),
                SeedQuestion(
                    "什么是 ReAct 模式？",
                    "ReAct 让 LLM 交替进行推理（Reasoning）与行动（Acting）：\n\n1) 思考：\"我需要查资料\"；\n2) 行动：调用工具/查询；\n3) 观察：把结果作为下一步思考的输入。\n\n形成\"思考→行动→观察\"循环直到得出答案。它结合了思维链的推理能力与工具交互能力，是 Agent 最常用的工作流之一。",
                    1,
                ),
                SeedQuestion(
                ...
                SeedQuestion(
                    "Agent 与普通 LLM 对话的区别？",
                    "普通对话是单轮问答，模型只做生成；Agent 具备自主性：\n\n1) 可调用外部工具获取信息；\n2) 执行动作；\n3) 依据反馈循环调整；\n4) 完成多步骤闭环任务。\n\nAgent 还引入记忆、规划与错误恢复，从\"会说话\"升级为\"会做事\"。",
                    1,
                ),
                SeedQuestion(
                    "什么是 Reflexion（自我反思）？",
                    "Reflexion 让 Agent 在失败后对自己的行为做语言化反思（总结哪里错了、下次怎么做），把反思写入记忆供后续尝试参考，从而在任务中不断自我修正。\n\n与 ReAct 结合形成\"执行→反馈→反思→再执行\"的循环，能显著提升复杂任务成功率。",
                    2,
                ),
                SeedQuestion(
                    "如何用 LangGraph 构建 Agent？",
                    "LangGraph 把 Agent 建模为图（节点=LLM/工具/条件边），支持循环与状态管理，比纯链式更适合有条件的多步 Agent。\n\n构建流程：\n\n1) 定义状态 schema；\n2) 添加节点（agent 决策、工具执行）；\n3) 连接边与条件边（是否继续/结束）；\n4) 用 checkpoint 持久化以支持断点续跑与人机交互。",
                    3,
                ),
            ),
        ),
        SeedCategory(
            ...
            questions = listOf(
                SeedQuestion(
                    ...
                    "Transformer 由多层 encoder/decoder 堆叠而成，每层包含多头自注意力（Multi-Head Self-Attention）与前馈网络（FFN），并带残差连接和层归一化。\n\n核心创新是注意力机制，摆脱了 RNN 的序列依赖，可并行训练并建模长距离依赖。现代 LLM（GPT、LLaMA 等）基本采用 decoder-only 架构。",
                    2,
                ),
                SeedQuestion(
                ...
                SeedQuestion(
                    "什么是位置编码？为什么需要它？",
                    "注意力机制本身对输入顺序不敏感（置换等变），无法感知 token 的先后关系，因此需要位置编码为每个位置注入顺序信息。\n\n原始 Transformer 用正弦/余弦绝对位置编码，现代模型常用旋转位置编码（RoPE）、ALiBi 等相对位置编码，支持更好的长上下文外推。",
                    2,
                ),
                SeedQuestion(
        ...
        SeedCategory(
            ...
            questions = listOf(
                ...
                SeedQuestion(
                    "什么是消息队列？应用场景有哪些？",
                    "消息队列是异步解耦的中间件（Kafka、RabbitMQ、RocketMQ）。\n\n应用场景：\n\n1) 异步削峰（秒杀、通知）；\n2) 服务解耦；\n3) 最终一致性（订单-库存-支付）；\n4) 流处理；\n5) 日志收集。\n\n要点： 消息不丢（ack/持久化）、不重（幂等消费）、不乱序、延迟可控。",
                    2,
                ),
                SeedQuestion(
                    "什么是水平扩展与垂直扩展？",
        ...
        SeedCategory(
            ...
            questions = listOf(
                ...
                SeedQuestion(
                    "什么是 PCA？",
                    "PCA（主成分分析）是无监督降维方法：通过对协方差矩阵做特征分解，找到数据方差最大的正交方向（主成分），把数据投影到前 K 个主成分上。\n\n作用： 降维可视化、去相关、压缩特征、缓解过拟合。\n\n代价： 损失部分信息、可解释性下降，使用前需标准化。",
                    2,
                ),
                SeedQuestion(
                    "什么是偏差-方差权衡？",
                    "泛化误差由偏差（Bias）、方差（Variance）与不可约噪声构成。\n\n1) 偏差高：模型过于简单（欠拟合）；\n2) 方差高：对训练数据过于敏感（过拟合）。\n\n模型复杂度上升时偏差下降而方差上升，存在最优平衡点。调节手段：正则化降方差，更强模型/更多特征降偏差。",
                    3,
                ),
            ),
        ),
