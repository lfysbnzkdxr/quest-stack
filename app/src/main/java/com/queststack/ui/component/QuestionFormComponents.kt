...
/** 分类选择（收缩式下拉，样式与题库筛选栏一致；选项：各分类 + 未分类）
...
fun DifficultySelector(
    ...
    val items = buildList {
        add(
            DropdownItem(text = "简单", selected = difficulty == 1, onClick = {
                onSelect(1)
                expanded = false
            }),
        )
        add(
            DropdownItem(text = "中等", selected = difficulty == 2, onClick = {
                onSelect(2)
                expanded = false
            }),
        )
        add(
            DropdownItem(text = "困难", selected = difficulty == 3, onClick = {
                onSelect(3)
                expanded = false
            }),
        )
